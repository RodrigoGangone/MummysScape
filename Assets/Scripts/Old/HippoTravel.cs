...
public class HippoTravel : MonoBehaviour, IPausable
{
    ...
    private IEnumerator Co_Travel(Transform player)
    {
        ...
        {
            if (_paused) { yield return PauseUtils.WaitWhilePaused(() => _paused); continue; }
        ...
        player.position = p1; player.rotation = r1;

        // 2) Abrir boca (start) + esperar 0.5s pausable
        if (HippoStartAnim) HippoStartAnim.SetTrigger("isOpen");
        yield return PauseUtils.WaitForSecondsPausable(0.5f, () => _paused);
    ...
    // (Opcional) hooks si los usás desde eventos de animación
    public void StartTravelCompleted() => Debug.Log("La primera parte del viaje se hizo con éxito");
    public void DestTravelCompleted()  => Debug.Log("La segunda parte del viaje se hizo con éxito");
}
