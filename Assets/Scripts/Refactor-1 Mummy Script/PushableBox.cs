...
{
    [Header("Setup")]
    [SerializeField] private float _halfExtent = 0.5f;
    ...
    [Header("Tuning")]
    [SerializeField, Min(0f)] private float _pushSpeed = 2.5f;
    ...
    #region IExclusiveInteractable
        ...
        public bool TryAcquire(Object user) { if (IsBusy) return false; _owner = user; return true; }
        public void Release(Object user) { if (_owner == user) _owner = null; }
     ...
     public bool TryGetPushInfo(Transform player, float maxFaceAngleDeg, float maxDist, out PushInfo info)
        {
            ...
            // El eje permitido es el más ortogonal a la normal de la cara mirado por el player.
            Vector3 faceNormal;
            Vector3 moveAxis;
            if (Mathf.Abs(dotX) > Mathf.Abs(dotZ))
            {
                faceNormal = Mathf.Sign(dotX) > 0 ? right : -right; // caras ±X
                moveAxis   = fwd; // si empujás contra ±X, se mueve en Z
            }
            else
            {
                faceNormal = Mathf.Sign(dotZ) > 0 ? fwd : -fwd; // caras ±Z
                moveAxis   = right; // si empujás contra ±Z, se mueve en X
            }
    
            // Ángulo máximo permitido entre mirada del player y la cara (para “pegado”)
            float angle = Vector3.Angle(player.forward, faceNormal * -1f);
            if (angle > maxFaceAngleDeg) return false;
    
            // Punto de snap (a ras de la cara)
        ...
        public void OnPushStart(in PushInfo info)
        {
            ...
            var freeze = _defaultConstraints | (moveIsX ? RigidbodyConstraints.FreezePositionZ : RigidbodyConstraints.FreezePositionX);
        ...
        public void OnPushEnd()
        {
            ...
            // frenar suavemente
            Vector3 v = _rb.velocity; v.x *= 0.1f; v.z *= 0.1f; _rb.velocity = v;
        }
    ...
    #region IAttractable
        ...
        public void OnAttractStart() { /* opcional fx */ }
    ...
    #region Visual helpers
        ...
        {
            if (_canFX) { if (can && !_canFX.isPlaying) _canFX.Play(); else if (!can && _canFX.isPlaying) _canFX.Stop(); }
        }
        public void SetHoverBlockedBySize(bool blocked)
        {
            if (_sizeFX) { if (blocked && !_sizeFX.isPlaying) _sizeFX.Play(); else if (!blocked && _sizeFX.isPlaying) _sizeFX.Stop(); }
        }
